...
- **vLLM** is the single-GPU production default: 94% MMLU accuracy, 30.2 tok/s
- **GPTQ** is the fastest HF mode: 34.7 tok/s, lowest TTFT (31 ms), 68% less VRAM than fp16
- **NF4** is the VRAM-constrained pick: 7.8 GB vs 17.3 GB for fp16, with comparable quality (74% MMLU)

**Scaling up — A100 & H100**
- **2× A100-80GB (tensor-parallel)**: throughput ceiling at 146.7 tok/s batch-1 and 1,106 tok/s at batch-8; cheapest per-token at scale ($0.0042/1k)
- **H100 (fp8)**: hardware-native FP8 is the recommended single-H100 mode — SW-emulated fp8 on A10G scores only 6% MMLU and should not be used
